...
public sealed unsafe class PhysicalVirtualMemory : IVirtualMemory, IGuestMemoryAllocator, IGuestAddressSpace, IDisposable
{
    ...
    // RAGE and other full C++ runtimes route large numbers of HLE-backed heap
    // allocations through this arena. The original 16 MiB reserve could be
    // exhausted during asset setup, turning a valid allocation request into a
    // null pointer that failed later in unrelated guest code. Reserving 512 MiB
    // costs virtual address space; host pages are only physically backed when
    // touched. Adapted from foufouadi's allocator-exhaustion investigation.
