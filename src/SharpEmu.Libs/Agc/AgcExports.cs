...
#pragma warning restore SHEM004, SHEM006
        ...
        out string description)
    {
        ...
        [
            "VLshlAddU32", "VMovB32", "VMovB32", "VMovB32", "VMovB32",
            "BufferStoreFormatXyzw", "SEndpgm",
        ];
        if (instructions.Count != expectedOpcodes.Length ||
            ...
            dispatch.BaseGroupX != 0 || dispatch.BaseGroupY != 0 || dispatch.BaseGroupZ != 0 ||
            dispatch.GroupCountY != 1 || dispatch.GroupCountZ != 1 ||
            localSizeX != 64 || localSizeY != 1 || localSizeZ != 1 ||
        ...
        var destination = evaluation.GlobalMemoryBindings.SingleOrDefault(
            static binding => binding.ScalarAddress == 0 && binding.Writable && binding.WriteBackToGuest);
        var scalars = evaluation.InitialScalarRegisters;
        if (destination is null || destination.BaseAddress == 0 ||
            destination.DataLength < FillRecordBytes || scalars.Count < 8 ||
        ...
        var dispatchedThreads = dispatch.ThreadCountX != uint.MaxValue
            ? dispatch.ThreadCountX
            : Math.Min((ulong)uint.MaxValue, (ulong)dispatch.GroupCountX * localSizeX);
        var writableRecords = (uint)(destination.DataLength / FillRecordBytes);
        var outputRecords = (uint)Math.Min(Math.Min((ulong)numRecords, dispatchedThreads), writableRecords);
        if (outputRecords == 0)
        {
            return false;
        }
    ...
    // RDNA2 BUF_FMT_32_32_32_32_UINT. The fast path copies the raw dwords.
    private const uint BufFmt32323232Uint = 75;

    private static bool IsExactConstantFillInstructionShape(
        IReadOnlyList<Gen5ShaderInstruction> instructions)
    {
        static bool IsOperand(Gen5Operand operand, Gen5OperandKind kind, uint value) =>
        ...
        for (var index = 0; index < 4; index++)
        {
            ...
            if (move.Destinations.Count != 1 ||
                !IsOperand(move.Destinations[0], Gen5OperandKind.VectorRegister, (uint)index) ||
                move.Sources.Count != 1 ||
                !IsOperand(move.Sources[0], Gen5OperandKind.ScalarRegister, (uint)(4 + index)))
            {
                return false;
            }
        }
        ...
        {
            DwordCount: 4, OffsetBytes: 0, IndexEnabled: true,
            OffsetEnabled: false, Glc: false, Slc: false,
        } control && control.VectorAddress == 4 && control.VectorData == 0 && control.ScalarResource == 0;
    }
        ...
        ulong expectedBaseAddress)
    {
        ...
        var matches = baseAddress == expectedBaseAddress && stride == FillRecordBytes &&
            !cacheSwizzle && !swizzleEnabled && unifiedFormat == BufFmt32323232Uint &&
            !addTidEnabled && outOfBoundsSelect == 0 && type == 0 && dstSelectX == 4;
        if (!matches && baseAddress == expectedBaseAddress && _traceAgcShader)
        {
            Console.Error.WriteLine(
                ...
                $"format={unifiedFormat} oob={outOfBoundsSelect} type={type} dst_sel_x={dstSelectX}");
        }
